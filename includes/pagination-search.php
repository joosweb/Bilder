<?php
    ...
    require('../db/conectar.php');
        ...
        $precioMinArray = array("0","50000","100.000","150000","200000","250000","300000","350000","400000","450000","500000","550000","600000","650000","700000","750000","800000","850000","900000","950000","1000000","1250000","1500000","1750000","2000000","2500000","3000000","4000000","5000000","10000000");

        $precioMaxArray = array("0","50000","100.000","150000","200000","250000","300000","350000","400000","450000","500000","550000","600000","650000","700000","750000","800000","850000","900000","950000","1000000","1250000","1500000","1750000","2000000","2500000","3000000","4000000","5000000","10000000","20000000");

     // CREAR UN QUERY SEGUN LA BUSQUEDA DEL FORMULARIO
       

        if ($_GET['region'] != '') {
            $region = $_GET['region'];
            $set_query = $set_query." and region='".$region."'";
        }
        if ($_GET['comuna'] != '') {
            $comuna = $_GET['comuna'];
            $set_query = $set_query." and comuna='".$comuna."'";
        }
        if ($_GET['tipo_inmueble'] != '') {
            $tipo_inmueble = $_GET['tipo_inmueble'];
            $set_query = $set_query." and tipo_inmueble='". $tipo_inmueble."'";
        }
        if ($_GET['precioMin'] != '') {
            $precioMIN = $_GET['precioMin'];
            $set_query = $set_query." and precio >= ".$precioMinArray[$precioMIN]."";
        }
        if ($_GET['precioMax'] != '') {
            $precioMAX = $_GET['precioMax'];
            $set_query = $set_query." and precio <= ".$precioMaxArray[$precioMAX]."";
        }
        if($_GET['palabra'] != '') {
            $palabra = $_GET['palabra'];
            $set_query = $set_query." and descripcion LIKE  '%".$palabra."%'";
        }
    ...
    $stm = $pdo->prepare("SELECT * FROM c_publisheds WHERE status=1 ".$set_query."");
    $stm->execute();
    ...
    $total_paginas = ceil($num_total_registros / $rowsPerPage);
        ...
        $stm2 = $pdo->prepare("SELECT * FROM c_publisheds WHERE status=1 ".$set_query." ORDER BY fecha_publicacion DESC, visitas DESC LIMIT $offset, $rowsPerPage");
        $stm2->execute();
